...
from services.chat_service import ChatServiceProtocol

router = APIRouter(prefix="/chat", tags=["chat"])


@router.post("", response_model=ChatResponse)
async def chat(req: ChatRequest, svc: ChatServiceProtocol = Depends(get_chat_service)):
    return await svc.send_message(req.user_id, req.message)


@router.get("/history", response_model=HistoryResponse)
def get_history(user_id: str, svc: ChatServiceProtocol = Depends(get_chat_service)):
    return {"history": svc.fetch_history(user_id)}


@router.delete("/history", response_model=MessageResponse)
def clear_history(user_id: str, svc: ChatServiceProtocol = Depends(get_chat_service)):
